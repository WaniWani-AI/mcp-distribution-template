...
- **Skybridge view filename ≠ registered name** -- The file at `src/views/<name>.tsx` MUST match the `view.component` passed to `registerTool({ name, view: { component: "<name>" } }, ...)` and the `tool` passed to `showWidget({ tool: "<name>" })`. A mismatch surfaces as `[vite] Failed to fetch dynamically imported module: .../src/views/<name>` in the dev-server log and the view never renders. When debugging "view not rendering," check the dev-server log first -- it names the exact path Vite is looking for. Each view file **default-exports** its component; skybridge mounts it (no `mountView(...)` call). Skybridge 1.x renamed widgets to views: `registerWidget()` is gone, and the Vite plugin moved from `skybridge/web` to `skybridge/vite`.
